...
int main(){
	...
	lidar->Listen([&new_scan, &scan_mutex, &lastScanTime, &scanCloud](auto data){
		...
		if(new_scan){
			...
			if(pclCloud.points.size() > 1200){ // Batch size for each scan-matching update.
	...
	while (!viewer->wasStopped())
  	{
		...
		if(has_scan){
			...
			voxelFilter.setLeafSize(0.7f, 0.7f, 0.7f);
			...
			passX.setFilterLimits(initialGuess(0, 3) - 20.0f, initialGuess(0, 3) + 20.0f);
			passX.filter(*localMapX);

			pcl::PassThrough<PointT> passY;
			passY.setInputCloud(localMapX);
			passY.setFilterFieldName("y");
			passY.setFilterLimits(initialGuess(1, 3) - 20.0f, initialGuess(1, 3) + 20.0f);
